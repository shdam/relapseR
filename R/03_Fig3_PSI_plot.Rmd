...
```{r}
message("Creating PSI plot")
colors <- c("hotpink3", "darkblue", "darkorange", "darkred", "darkgreen")
psi_plot <- psi_icope %>%
  ...
  rbind(psi_sra_cll) %>%
  rbind(psi_sra_dlbcl) %>%
  mutate(type = factor(type, levels = c("B-ALL", "DLBCL", "CLL", "MM", "Blood", "Spleen"))) |>  #, "CLL"
  ggplot() +
...
# ggsave(psi_plot, filename = "../figs/PSI_SRA_full.png", width = 10, height = 10)
```
