...
if (!exists("../data/md_tcga.Rdata")) {
  md_tcga <- snapcount:::get_compilation_metadata(compilation = "tcga")
...
md_sra_ball <- md_sra |> 
  dplyr::filter(stringr::str_detect(study_title, "B-ALL"),
                stringr::str_detect(sample_attributes, "leukemia"))
...
```{r}
message("Creating PSI plot")
colors <- c("hotpink3", "darkblue", "darkred", "darkgreen", "lightblue")
psi_plot <- psi_icope %>%
  ...
  rbind(psi_sra_dlbcl) %>%
  mutate(type = factor(type, levels = c("iCOPE B-ALL", "SRA DLBCL", "GTEx Blood", "GTEx Spleen"))) |> 
  ggplot() +
  ...
  xlim(0.6, 1.05)
psi_plot
ggsave(psi_plot, filename = "../figs/PSI_SRA_hist.pdf", width = 10, height = 5)
